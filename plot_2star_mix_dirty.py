...
files = [
    "dirty/DIRTY_bm1_effgrain_tau_001.00_theta_000_mod" + k + "_global_lum.table.fits"
    for k in filetag
]
...
for k, cfile in enumerate(files):
    ...
    ax[1].plot(
        1.0 / x,
        tau_eff_direct / tau_eff_direct[sindxs[0]],
    ...
    ax[2].plot(
        1.0 / x,
        c_output_scat / c_input,
        label=label_str.format(av1[k], tau_eff_direct[sindxs[0]]),
    )

ax[0].plot(
    1.0 / x, a["tau_norm"] / a["tau_norm"][sindxs[0]], "k--", label="input extinction"
)
ax[1].plot(
    1.0 / x, a["tau_norm"] / a["tau_norm"][sindxs[0]], "k--", label="input extinction"
)
...
for i in range(3):
    ...
    ax[i].set_xscale("log")
    ax[i].set_xlim(1.0 / 10.0, 1.0 / 0.3)
...
ax[3].plot([-2.0, -5.0, -5.0, -2.0, -2.0], [-5.0, -5.0, 5.0, 5.0, -5.0], "k--")
...
for k in range(len(ann_x)):
    if k % 2 == 0:
        ...
        valign = "center"
    ax[3].annotate(
        "Star #1, Model" + str(k + 1),
        xy=(ann_x[k], ann_y[k]),
        xytext=(ann_x[k] + 2.0, yval),
        rotation=90.0,
...
ax[3].plot(
    [5.0, 3.8, 4.0, 3.85, 3.8, 3.85, 4.0, 3.8, 5.0],
    [0.0, 1.1, 0.925, 0.5, 0.0, -0.5, -0.925, -1.1, 0.0],
    "k-",
)
ax[3].plot([3.85, 4.05, 4.1, 4.05, 3.85], [0.25, 0.125, 0.0, -0.125, -0.25], "k-")
